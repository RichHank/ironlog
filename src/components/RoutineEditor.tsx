    ...
    <div className="fixed inset-0 z-50 flex flex-col bg-zinc-950 animate-fade-in">
      <div className="safe-area-top border-b border-zinc-800 bg-zinc-950 px-4 pb-3">
        <div className="flex items-center justify-between gap-3 mb-3">
          <h2 className="text-lg font-bold text-zinc-50">{routine ? 'Edit Routine' : 'New Routine'}</h2>
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-8">
        ...
        {draft.exercises.map((ex, ei) => (
          <div key={ex.id} className="mt-3 card p-4">
            <div className="flex items-center justify-between gap-2 mb-3">
              <p className="text-sm font-bold text-zinc-50">{ei + 1}. {ex.name}</p>
              <button onClick={() => removeExercise(ex.id)} className="text-xs text-zinc-600 hover:text-red-400">Remove</button>
            </div>
            {/* Planned sets */}
            <div className="space-y-2">
              {ex.plannedSets.map((set, si) => (
                <div key={set.id} className="flex items-center gap-2">
                  <span className="text-xs text-zinc-500 w-12">Set {si + 1}</span>
                  <input
                  ...
                  {ex.plannedSets.length > 1 && (
                    <button onClick={() => removePlannedSet(ex.id, set.id)} className="text-xs text-zinc-600 hover:text-red-400 px-1">×</button>
                  )}
                </div>
              ))}
            </div>
        ...
        <button
          ...
          className="mt-3 card flex min-h-touch w-full items-center justify-center gap-2 py-3 font-semibold text-blue-400 active:bg-zinc-800/40"
        >
          + Add Exercise
        </button>
        ...
        {showExSearch && (
          <div className="fixed inset-0 z-[60] flex flex-col bg-zinc-950 animate-fade-in">
            <div className="safe-area-top border-b border-zinc-800 px-4 pb-3">
              <div className="flex items-center justify-between gap-3 mb-3">
                <h3 className="text-lg font-bold text-zinc-50">Add Exercise</h3>
      ...
      {/* Save button */}
      <div className="safe-area-bottom border-t border-zinc-800 bg-zinc-950 px-4 py-3">
        <button
          onClick={handleSave}
