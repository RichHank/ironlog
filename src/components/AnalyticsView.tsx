...
export default function AnalyticsView({ sessions }: Props) {
  ...
  return (
    <div className="px-3 pt-4 sm:px-4 pb-4">
      <div className="mb-4">
        <p className="text-xs text-zinc-500 uppercase tracking-wider">Analytics</p>
        <p className="text-lg font-black text-zinc-50">Training Stats</p>
      </div>
        ...
        {(['7d', '30d', '90d', 'all'] as const).map(p => (
          <button
            ...
            className={`rounded-full px-4 py-1.5 text-xs font-semibold ${
              selectedPeriod === p ? 'bg-blue-500 text-white' : 'bg-zinc-800 text-zinc-400'
            }`}
          >
      ...
      <div className="grid grid-cols-3 gap-3 mb-4">
        <div className="card p-3 text-center">
          ...
          <p className="text-xl font-black text-zinc-50 mt-1">{stats.totalWorkouts}</p>
        </div>
        <div className="card p-3 text-center">
          <p className="text-xs text-zinc-500 uppercase tracking-wider">Sets</p>
          <p className="text-xl font-black text-zinc-50 mt-1">{stats.totalSets}</p>
        </div>
        <div className="card p-3 text-center">
          <p className="text-xs text-zinc-500 uppercase tracking-wider">Volume</p>
          <p className="text-lg font-black text-blue-400 mt-1">{stats.totalVolume.toLocaleString()}</p>
        </div>
      </div>

      {/* Muscle group distribution */}
      <div className="card p-4 mb-4">
        <p className="text-xs text-zinc-500 uppercase tracking-wider mb-3">Muscle Group Volume</p>
        <div className="space-y-2">
          {topMuscles.map(([muscle, volume]) => (
            <div key={muscle} className="flex items-center gap-3">
              <p className="text-xs text-zinc-400 w-20 flex-shrink-0">{muscle}</p>
              ...
              <p className="text-xs font-mono text-zinc-300 w-16 text-right">{volume.toLocaleString()}</p>
            </div>
          ))}
          {topMuscles.length === 0 && (
            <p className="text-xs text-zinc-500 text-center py-4">No data for this period</p>
          )}
        </div>
      </div>
      ...
      <div className="card p-4 mb-4">
        <p className="text-xs text-zinc-500 uppercase tracking-wider mb-2">Exercise Trend</p>
        <select
        ...
        {selectedExercise && exerciseTrend.length > 0 && (
          <div>
              ...
              {exerciseTrend.map((point, i) => {
                ...
                return (
                  <div key={i} className="flex-1 flex flex-col items-center justify-end h-full">
                    <div
                      className="w-full rounded-t bg-blue-500 transition-all min-h-[2px]"
                      style={{ height: `${height}%` }}
                    />
                  </div>
                );
            ...
            <div className="flex justify-between mt-1">
              <span className="text-[9px] text-zinc-600">{exerciseTrend[0]?.date}</span>
              <span className="text-[9px] text-zinc-600">{exerciseTrend[exerciseTrend.length - 1]?.date}</span>
            </div>
          </div>
        )}
        {selectedExercise && exerciseTrend.length === 0 && (
          <p className="text-xs text-zinc-500 py-4 text-center">No data for this exercise</p>
        )}

        {/* PRs for selected exercise */}
        {selectedExercise && exercisePRs.length > 0 && (
          <div className="mt-4">
            <p className="text-xs text-zinc-500 uppercase tracking-wider mb-2">Personal Records</p>
            <div className="space-y-2">
              {exercisePRs.map(pr => (
                <div key={pr.id} className="flex items-center justify-between rounded-lg bg-zinc-800/50 px-3 py-2">
                  <div>
                    ...
                    <p className="text-[10px] text-zinc-500">{new Date(pr.achievedAt).toLocaleDateString()}</p>
                  </div>
